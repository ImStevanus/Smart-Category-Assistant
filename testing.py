...
# 2. CUSTOM CSS (SUPORT DARK & LIGHT MODE)
st.markdown("""
    <style>
    /* Styling kotak metrik agar adaptif */
    ...
    /* Memastikan teks nilai metrik kontras */
...
# 3. FUNGSI LOAD ASSETS & PENJELASAN CLUSTER
@st.cache_resource
def load_assets():
    try:
...
def get_cluster_info(cluster_num):
    info = {
        0: {
            ...
            "saran": "Pertahankan ritme belajar. Sangat direkomendasikan menjadi asisten dosen.",
            "color": "#2ecc71"
        },
        1: {
            "label": "📊 Steady / Average (Performa Menengah)",
            "desc": "Mahasiswa menunjukkan performa stabil namun ada ruang untuk peningkatan di aspek praktikum/lab.",
            "saran": "Fokus meningkatkan nilai tugas harian untuk mengamankan nilai akhir.",
            "color": "#f1c40f"
        },
        2: {
            "label": "⚠️ Underperformer (Performa Berisiko)",
            "desc": "Nilai ujian rendah dan tingkat kehadiran di bawah batas minimum. Berisiko tinggi dropout.",
...
if model is None or scaler is None:
    ...
    if menu == "🏠 Dashboard Analisis":
        ...
        if uploaded_file is not None:
            ...
            # BAGIAN METRIK (Sudah Diperbaiki untuk Dark Mode)
            ...
            with col_left:
                ...
                fig = px.scatter(df, x="midterm_marks", y="final_marks", color="Cluster", 
                                 template="none", # Mengikuti tema Streamlit
                                 color_continuous_scale="Viridis")
                st.plotly_chart(fig, use_container_width=True)

            with col_right:
                st.subheader("🥧 Proporsi Kelompok")
                df['Cluster_Name'] = df['Cluster'].apply(lambda x: get_cluster_info(x)['label'])
            ...
            st.info("Silakan unggah dataset 'student_dropout_behavior_dataset.csv' pada sidebar untuk melihat analisis.")

    # --- MENU 2: PREDIKSI INDIVIDU ---
    else:
        st.title("🔍 Prediksi Kategori Mahasiswa")
        st.write("Gunakan slider untuk memasukkan data mahasiswa.")
        ...
        with st.form("prediction_form"):
            ...
            with c1:
                ...
                q3 = st.slider("Quiz 3", 0.0, 10.0, 7.5)
                mid = st.number_input("Midterm Marks", 0, 100, 70)
            with c2:
                fin = st.number_input("Final Marks", 0, 100, 75)
                gpa = st.slider("Previous GPA", 0.0, 4.0, 3.2)
